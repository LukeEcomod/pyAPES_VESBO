# -*- coding: utf-8 -*-
"""
Created on Sun Jan 07 16:20:49 2018

@author: slauniai

FUNCTIONS FOR FLUX-DATA ANALYSIS
"""
...
def running_mean(data, N, center=True):
    """
    N-element running mean of pd.Dataframe or np.ndarray
    Args:
        data - pd.DataFrame or np.ndarray
        N - number of consequtive points to be averaged
        center - True centers average to window midpoint;
                 False sets average to window right boundary
    Returns:
        res - averaged pd.DataFrame or np.ndarray. center=True leaves
              first and last N/2 points as np.NaN, center=False first N-1 points
    Samuli Launiainen, Luke 8th Jan, 2018
    """

    if isinstance(data, pd.DataFrame) or isinstance(data, pd.Series):
        res = data.rolling(N, center=center, win_type=None).mean()

    elif isinstance(data, np.ndarray):

        if data.ndim == 1:  # vector
            res = np.ones(len(data)) * np.NaN
            ...
            r, c = np.shape(data)
            res = np.ones((r, c)) * np.NaN

            for k in range(c):
                cumsum = np.cumsum(np.insert(data[:, k], 0, 0))
                ave = (cumsum[N:] - cumsum[:-N]) / float(N)
                if center:
                    res[N/2:-(N/2-1), k] = ave
                else:
...
"""
Mann-Kendall test by Michael Schramm
